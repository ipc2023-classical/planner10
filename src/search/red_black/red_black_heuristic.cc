...
int RedBlackHeuristic::compute_heuristic(const State &global_state) {
    initialize();
    ...
    State state = convert_ancestor_state(global_state);
...
bool RedBlackHeuristic::is_currently_RB_applicable(const vector<int>& ops) const {
    ...
    if (ops.size() == 1) {
        return op_all_red_preconditions_reached(ops[0]);
    }
...
const vector<int>& RedBlackHeuristic::get_path_for_var_from_to(VariableProxy var, int from, int to) const {
    // First, trying to find a sequence based only on current values
    if (applicability_status) {
        get_dtg(var)->set_transitions_enablement_status(ONLY_CURRENT_TRANSITIONS);
...
int RedBlackHeuristic::compute_sequential_relaxed_plan(const State &state) {
    ...
    for (size_t i = 0; i < goal_propositions.size(); ++i)
        get_relaxed_plan(state, &propositions[goal_propositions[i]]);
...
void RedBlackHeuristic::get_relaxed_plan(const State &state,
    ...
    if (!goal->marked) { // Only consider each subgoal once.
        ...
        if (op_id != relaxation_heuristic::NO_OP) { // We have not yet chained back to a start node.
            ...
            for (int i = 0; i < unary_op->num_preconditions; ++i)
                //get_relaxed_plan(state, unary_op->preconditions[i]); omg
                get_relaxed_plan(state, get_proposition(get_preconditions_vector(op_id)[i]));
            ...
            if (operator_no != -1) {
                ...
                if (conditional_effects_task && red_black_task.operator_has_red_conditional_effects(operator_no)) {
                    ...
                    propositions_per_operator[operator_no][unary_op->effect] = true;
                }
